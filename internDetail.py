...
def show_intern_profile(intern):
    ...
    with st.expander(" Remove Intern (Danger Zone)"):
        ...
        # 1. CSS trick to make the Intern Name unselectable (Cannot be highlighted/copied)
        st.markdown(
            f"To confirm, type <span style='user-select: none; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; pointer-events: none; font-weight: bold; color: #ffffff;'>{intern['Name']}</span> below:", 
            unsafe_allow_html=True
        )
